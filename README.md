...
- example_app - Code for the application's Lambda function.
...
```bash
fast-api$ sam build --use-container
```
...
```bash
fast-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
fast-api$ sam local start-api
fast-api$ curl http://localhost:3000/
```
...
```bash
fast-api$ sam logs -n HelloWorldFunction --stack-name fast-api --tail
```
...
```bash
fast-api$ pip install pytest pytest-mock --user
fast-api$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name fast-api
```
