...
EVENT_BODY = (
    ...
    if len(sys.argv) > 2
        ...
        (
            sys.stdin.read()
            if os.environ.get("DOCKER_LAMBDA_USE_STDIN", False)
            else "{}"
        ),
    )
)
...
DEADLINE_MS = int(time.time() * 1000) + int(
    os.environ.get("AWS_LAMBDA_FUNCTION_TIMEOUT", "300")
)
REGION = os.environ.get("AWS_REGION", os.environ.get("AWS_DEFAULT_REGION", "us-east-1"))
ACCOUNT_ID = os.environ.get(
    "AWS_ACCOUNT_ID", random.randint(100000000000, 999999999999)
)
...
THROTTLED = False
DATA_SOCK = -1
CONTEXT_OBJS = {"clientcontext": None, "cognitoidentityid": None, "cognitopoolid": None}
CREDENTIALS = {
    "key": ACCESS_KEY_ID,
    ...
    while True:
        try:
            ...
            if time.time() > ping_timeout:
                ...
                time.sleep(0.005)
                continue
    return (INVOKEID, INVOKE_MODE, HANDLER, SUPPRESS_INIT, THROTTLED, CREDENTIALS)
...
def receive_invoke():
    ...
    INVOKED_FUNCTION_ARN = resp.getheader("Lambda-Runtime-Invoked-Function-Arn")
    XRAY_TRACE_ID = resp.getheader("Lambda-Runtime-Trace-Id")
    cognito_identity = json.loads(
        resp.getheader("Lambda-Runtime-Cognito-Identity", "{}")
    )
    ...
    CONTEXT_OBJS["clientcontext"] = resp.getheader("Lambda-Runtime-Client-Context")
...
def report_done(invokeid, errortype, result, is_fatal):
    ...
    MOCKSERVER_CONN.request(
        "POST",
        "/2018-06-01/runtime/invocation/%s/%s"
        % (invokeid, "response" if errortype is None else "error"),
        result,
        headers,
    )
    resp = MOCKSERVER_CONN.getresponse()
